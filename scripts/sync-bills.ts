...
async function main() {
  ...
  console.log(`열린국회정보에서 최근 의안 ${billCount}건 조회 중...`);
  const bills = await fetchRecentBills(billCount);
  console.log(`${bills.length}건 조회됨`);
  ...
  console.log(
    `\n완료: 법안 ${billsUpserted}건 upsert, 표결 조회 대상 ${voteBillsProcessed}건, 표결 ${votesUpserted}건 upsert`,
  );
}
